...
const SimpleCard = ({ image, title, description, showButton }: { image: string, title: string, description: string, showButton: boolean }) => {
    return (
        <div className='flex-1 max-w-[400px] max-sm:max-w-[360px]  bg-gradient-to-br  from-[#CFCFCF]/50 to-[#6B6B6B]/30   rounded-lg max-sm:border border-primary-gray'>
            ...
            <div className=" w-[400px] max-sm:w-[360px] text-white flex flex-col p-2   bg-black/60   rounded-lg">
                <div className=" flex justify-center relative  items-center rounded-2xl">
                    ...
                    />
                    {showButton && <Button variant="secondary" size="sm" className='absolute right-2 top-2 bg-black/90 max-sm:bg-white/90 max-sm:text-black flex items-center gap-2'>
                        <p className='text-white max-sm:text-black'>Find Itineraries</p>
