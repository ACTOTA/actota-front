...
import { useBookingById } from '@/src/hooks/queries/account/useBookingsQuery';
...
function BookingDetails() {
  ...
  // Fetch booking data using the ID from the URL
  const { data: dataBooking, isLoading: isLoadingBooking } = useBookingById(bookingId);
  
  // Fetch itinerary data once we have the booking data
  const itineraryId = dataBooking && (dataBooking.itinerary_id as { $oid: string })?.$oid;
  const { data: dataItinerary, isLoading: isLoadingItinerary } = useItineraryById(itineraryId || '');

  const isLoading = isLoadingBooking || isLoadingItinerary;

  if (isLoading) {
    return <div className="text-white p-8 text-center">Loading booking details...</div>;
  }
  
  if (!dataBooking || !dataItinerary) {
    return <div className="text-white p-8 text-center">No booking details available.</div>;
  }
